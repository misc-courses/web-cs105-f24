...
debate about the best approach to CSS is a debate about _separation of
concerns_. [Separation of
...
Things get more complex with objects, arrays, and other data structures (okay, they are all objects). These can be declared constant, but that only means that the _reference_ to the memory location stays constant. The stored value can be changed. This is legal:
...
- To make state update _pure_, _replace_ instead of _modify_
...
Subtype Requirement: Let ϕ(x) be a property provable about objects _x_ of type
_T_. Then ϕ(y) should be true for objects _y_ of type _S_ where _S_ is a
subtype of _T_.

Or alternately if _S_ is a subtype of _T_, I should be able to use an _S_
everywhere I use a _T_.
