...
1. First make sure our repository is up-to-date with any upstream changes, by checking out `master` and pulling any recent changes. _(Note that there shouldn't be any changes to the master -- this is just to give you the process to go through when others may be adding things to master)_.
   ```
   git checkout master
